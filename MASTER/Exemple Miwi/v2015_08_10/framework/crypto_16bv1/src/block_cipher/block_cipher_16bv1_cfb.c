...
typedef enum
{
    BLOCK_CIPHER_CFB_STATE_IDLE = 0,
    BLOCK_CIPHER_CFB_STATE_INIT,
    BLOCK_CIPHER_CFB_STATE_PROCESS_DATA,
    BLOCK_CIPHER_CFB_STATE_ADD_DATA,
    BLOCK_CIPHER_CFB_STATE_WAIT_FOR_HW,
    BLOCK_CIPHER_CFB_STATE_GENERATE_KEYSTREAM,
    BLOCK_CIPHER_CFB_STATE_WAIT_FOR_KEYSTREAM,
} BLOCK_CIPHER_CFB_STATE;

void BLOCK_CIPHER_CFB_16BV1_Tasks (BLOCK_CIPHER_HANDLE handle);

BLOCK_CIPHER_ERRORS BLOCK_CIPHER_16BV1_CFB_Initialize (BLOCK_CIPHER_HANDLE handle, BLOCK_CIPHER_16BV1_CFB_CONTEXT * context, BLOCK_CIPHER_FunctionEncrypt encryptFunction, BLOCK_CIPHER_FunctionDecrypt decryptFunction, uint32_t blockSize, uint8_t * initializationVector, void * key, CRYPTO_KEY_TYPE keyType, CRYPTO_KEY_MODE keyMode)
{
    ...
    context->state = BLOCK_CIPHER_CFB_STATE_IDLE;
    ...
    pBlock->tasks = BLOCK_CIPHER_CFB_16BV1_Tasks;
...
{
    BLOCK_CIPHER_16BV1_CFB_CONTEXT * context = (BLOCK_CIPHER_16BV1_CFB_CONTEXT *)BLOCK_CIPHER_16BV1_HandleResolve(handle);
    BLOCK_CIPHER_OBJECT *pBlock = &blockHandles[handle];
    ...
    *numCipherBytes = 0;
    ...
    if((numCipherBytes>0)&&(pBlock->bytesRemaining==0))
    {
        ...
        {
            context->state = BLOCK_CIPHER_CFB_STATE_ADD_DATA;
        }
        else
        {
            pBlock->operation = BLOCK_CIPHER_OPERATION_ENCRYPT;
            context->state = BLOCK_CIPHER_CFB_STATE_INIT;
        }
    }
...
BLOCK_CIPHER_ERRORS BLOCK_CIPHER_16BV1_CFB_Decrypt (BLOCK_CIPHER_HANDLE handle, uint8_t * plainText, uint32_t * numPlainBytes, uint8_t * cipherText, uint32_t numCipherBytes, uint32_t options)
{
    BLOCK_CIPHER_16BV1_CFB_CONTEXT * context = (BLOCK_CIPHER_16BV1_CFB_CONTEXT *)BLOCK_CIPHER_16BV1_HandleResolve(handle);
    BLOCK_CIPHER_OBJECT *pBlock = &blockHandles[handle];
...
void BLOCK_CIPHER_CFB_16BV1_Tasks (BLOCK_CIPHER_HANDLE handle)
{
    ...
    do
    {
        ...
        {
            case BLOCK_CIPHER_CFB_STATE_INIT:
            ...
            case BLOCK_CIPHER_CFB_STATE_ADD_DATA:
                if (pBlock->inCount == 0)
                {
                ...
                else
                {
                    ...
                    context->state = BLOCK_CIPHER_CFB_STATE_PROCESS_DATA;
                }

                break;
                
            case BLOCK_CIPHER_CFB_STATE_PROCESS_DATA:
                ...
                CRYCONLbits.CRYGO = 1;
                context->state = BLOCK_CIPHER_CFB_STATE_WAIT_FOR_HW;
                break;
                
            case BLOCK_CIPHER_CFB_STATE_WAIT_FOR_HW:
                if(pBlock->inCount > blockSize)
                {
                    ...
                    pBlock->source += blockSize;
                    pBlock->inCount -= blockSize;
                }
                else
                {
                    memcpy (pBlock->dest, pBlock->outputText, pBlock->inCount);
                    memcpy (pBlock->previousData, pBlock->outputText, blockSize);
                    pBlock->bytesRemaining = blockSize - pBlock->inCount;
                    pBlock->dest += pBlock->inCount;
                    if(pBlock->outCount != NULL)
                    {
                        *pBlock->outCount += pBlock->inCount;
                    }
                    pBlock->source += pBlock->inCount;
                    pBlock->inCount -= pBlock->inCount;
                }
                context->state = BLOCK_CIPHER_CFB_STATE_ADD_DATA;
                break;
    ...
    } while ((context->state != BLOCK_CIPHER_CFB_STATE_WAIT_FOR_HW) && (context->state != BLOCK_CIPHER_CFB_STATE_IDLE));
