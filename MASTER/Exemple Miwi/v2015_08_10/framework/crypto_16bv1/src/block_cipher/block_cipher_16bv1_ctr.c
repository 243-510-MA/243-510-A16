...
typedef enum
{
    BLOCK_CIPHER_CTR_STATE_IDLE = 0,
    BLOCK_CIPHER_CTR_STATE_INIT,
    BLOCK_CIPHER_CTR_STATE_PROCESS_DATA,
    BLOCK_CIPHER_CTR_STATE_ADD_DATA,
    BLOCK_CIPHER_CTR_STATE_WAIT_FOR_HW,
    BLOCK_CIPHER_CTR_STATE_GENERATE_KEYSTREAM,
    BLOCK_CIPHER_CTR_STATE_WAIT_FOR_KEYSTREAM,
} BLOCK_CIPHER_CTR_STATE;

void BLOCK_CIPHER_CTR_16BV1_Tasks(BLOCK_CIPHER_HANDLE handle);
...
BLOCK_CIPHER_ERRORS BLOCK_CIPHER_16BV1_CTR_Initialize (BLOCK_CIPHER_HANDLE handle, BLOCK_CIPHER_16BV1_CTR_CONTEXT * context, BLOCK_CIPHER_FunctionEncrypt encryptFunction, BLOCK_CIPHER_FunctionDecrypt decryptFunction, uint32_t blockSize, uint8_t * noncePlusCounter, void * keyStream, uint32_t keyStreamSize, void * key, CRYPTO_KEY_TYPE keyType, CRYPTO_KEY_MODE keyMode)
{
    ...
    pBlock->mode = BLOCK_CIPHER_MODE_CTR;
    pBlock->state = BLOCK_CIPHER_STATE_IDLE;
    pBlock->error = BLOCK_CIPHER_ERROR_NONE;
    pBlock->cryconl_context.CPHRMOD = 4;   // CTR
    pBlock->cryconh_context.CTRSIZE = 0;   // None for now
    memcpy (context->counter, noncePlusCounter, blockSize);
    ...
    pBlock->tasks = BLOCK_CIPHER_CTR_16BV1_Tasks;
...
BLOCK_CIPHER_ERRORS BLOCK_CIPHER_16BV1_CTR_Encrypt (BLOCK_CIPHER_HANDLE handle, uint8_t * cipherText, uint32_t * numCipherBytes, uint8_t * plainText, uint32_t numPlainBytes, uint32_t options)
{
    BLOCK_CIPHER_OBJECT *pBlock = &blockHandles[handle];
    BLOCK_CIPHER_16BV1_CTR_CONTEXT * context = (BLOCK_CIPHER_16BV1_CTR_CONTEXT *)BLOCK_CIPHER_16BV1_HandleResolve(handle);
    ...
    if (pBlock->cryconh_context.CTRSIZE == 0)
    {
        if (options & BLOCK_CIPHER_OPTION_CTR_32BIT)
        {
            pBlock->cryconh_context.CTRSIZE = 31;
        }
        else if (options & BLOCK_CIPHER_OPTION_CTR_64BIT)
        {
            pBlock->cryconh_context.CTRSIZE = 63;
        }
        else
        {
            // Default - 129 bits
            pBlock->cryconh_context.CTRSIZE = 127;
        }
    }

    if (BLOCK_CIPHER_CurrentHandleIsInitialized())
    {
        context->state = BLOCK_CIPHER_CTR_STATE_PROCESS_DATA;
    }
    else
    {
        context->state = BLOCK_CIPHER_CTR_STATE_INIT;
...
void BLOCK_CIPHER_CTR_16BV1_Tasks(BLOCK_CIPHER_HANDLE handle)
{
    ...
    if (context == NULL)
    {
        ...
        context->state = BLOCK_CIPHER_CTR_STATE_IDLE;
        return;
    }
    ...
    do
    {
        ...
        {
            case BLOCK_CIPHER_CTR_STATE_INIT:
                ...
                {
                    context->state = BLOCK_CIPHER_CTR_STATE_GENERATE_KEYSTREAM;
                }
                
                memcpy (pBlock->previousText, context->counter, pBlock->blockSize);
                BLOCK_CIPHER_CurrentHandleInitialize();
                break;
            case BLOCK_CIPHER_CTR_STATE_PROCESS_DATA:
                {
                    uint8_t * output;
                    uint8_t * input;
                    ...
                    if (pBlock->inCount != 0)
                    {
                        context->state = BLOCK_CIPHER_CTR_STATE_ADD_DATA;
                    }
                    else
                    {
                ...
                break;
            case BLOCK_CIPHER_CTR_STATE_ADD_DATA:
                dataCount = blockSize;
                ...
                pBlock->source += dataCount;
                CRYCONLbits.CRYGO = 1;
                context->state = BLOCK_CIPHER_CTR_STATE_WAIT_FOR_HW;
                break;
            case BLOCK_CIPHER_CTR_STATE_WAIT_FOR_HW:
                dataCount = blockSize;
                if (pBlock->inCount < blockSize)
                {
                    dataCount = pBlock->inCount;
                ...
                break;
            case BLOCK_CIPHER_CTR_STATE_GENERATE_KEYSTREAM:
                ...
                CRYCONLbits.CRYGO = 1;
                context->state = BLOCK_CIPHER_CTR_STATE_WAIT_FOR_KEYSTREAM;
                break;
            case BLOCK_CIPHER_CTR_STATE_WAIT_FOR_KEYSTREAM:
    ...
    } while ((context->state != BLOCK_CIPHER_CTR_STATE_WAIT_FOR_HW) && (context->state != BLOCK_CIPHER_CTR_STATE_IDLE) && (context->state != BLOCK_CIPHER_CTR_STATE_WAIT_FOR_KEYSTREAM));
