...
typedef enum
{
    BLOCK_CIPHER_CBC_STATE_IDLE = 0,
    BLOCK_CIPHER_CBC_STATE_INIT,
    BLOCK_CIPHER_CBC_STATE_KEY_DECRYPT,
    BLOCK_CIPHER_CBC_STATE_PROCESS_DATA,
    BLOCK_CIPHER_CBC_STATE_ADD_DATA,
    BLOCK_CIPHER_CBC_STATE_WAIT_FOR_HW,
} BLOCK_CIPHER_CBC_STATE;

void BLOCK_CIPHER_CBC_16BV1_Tasks (BLOCK_CIPHER_HANDLE handle);
...
{
    BLOCK_CIPHER_OBJECT *pBlock = &blockHandles[handle];
    BLOCK_CIPHER_16BV1_CBC_CONTEXT * context = (BLOCK_CIPHER_16BV1_CBC_CONTEXT *)BLOCK_CIPHER_16BV1_HandleResolve(handle);
    ...
    *numCipherBytes = 0;
    ...
    {
        context->state = BLOCK_CIPHER_CBC_STATE_ADD_DATA;
    }
    else
    {
        context->state = BLOCK_CIPHER_CBC_STATE_INIT;
        pBlock->operation = BLOCK_CIPHER_OPERATION_ENCRYPT;
    }
    pBlock->state = BLOCK_CIPHER_STATE_BUSY;
    pBlock->tasks = BLOCK_CIPHER_CBC_16BV1_Tasks;
...
void BLOCK_CIPHER_CBC_16BV1_Tasks (BLOCK_CIPHER_HANDLE handle)
{
    ...
    if (context == NULL)
    {
        ...
        context->state = BLOCK_CIPHER_CBC_STATE_IDLE;
        return;
    }
    ...
    do
    {
        ...
        {
            case BLOCK_CIPHER_CBC_STATE_INIT:
                ...
                if (pBlock->operation == BLOCK_CIPHER_OPERATION_DECRYPT)
                {
                    if (pBlock->cryconl_context.CPHRSEL==1)
                    {
                        ...
                        CRYCONLbits.OPMOD = 2;
                        context->state = BLOCK_CIPHER_CBC_STATE_KEY_DECRYPT;
                        CRYCONLbits.CRYGO = 1;
                    }
                    else
                    {
                ...
                break;
            case BLOCK_CIPHER_CBC_STATE_KEY_DECRYPT:
                // Key decryption complete
                CRYCONLbits.OPMOD = 1;
                memcpy (pBlock->previousText, pBlock->previousData, blockSize);
                context->state = BLOCK_CIPHER_CBC_STATE_ADD_DATA;
                break;
            case BLOCK_CIPHER_CBC_STATE_ADD_DATA:
                if (pBlock->inCount == 0)
                {
                    ...
                    {
                        pBlock->dataGenerated = false;
                        memcpy (pBlock->previousData, pBlock->outputText, pBlock->blockSize);
                    }
                    pBlock->state = BLOCK_CIPHER_STATE_IDLE;
                    context->state = BLOCK_CIPHER_CBC_STATE_IDLE;
                    return;
                ...
                else
                {
                    if (pBlock->inCount < blockSize - pBlock->bytesRemaining)
                    {
                        ...
                        if ((pBlock->options & BLOCK_CIPHER_OPTION_STREAM_COMPLETE) == BLOCK_CIPHER_OPTION_STREAM_COMPLETE)
                        {
                            ...
                            context->state = BLOCK_CIPHER_CBC_STATE_PROCESS_DATA;
                        }
                        else
                        {
                ...
                break;
            case BLOCK_CIPHER_CBC_STATE_PROCESS_DATA:
                ...
                CRYCONLbits.CRYGO = 1;
                context->state = BLOCK_CIPHER_CBC_STATE_WAIT_FOR_HW;
                break;
            case BLOCK_CIPHER_CBC_STATE_WAIT_FOR_HW:
    ...
    } while ((context->state != BLOCK_CIPHER_CBC_STATE_WAIT_FOR_HW) && (context->state != BLOCK_CIPHER_CBC_STATE_IDLE) && (context->state != BLOCK_CIPHER_CBC_STATE_KEY_DECRYPT));
