...
BLOCK_CIPHER_SW_ERRORS BLOCK_CIPHER_SW_OFB_Encrypt (BLOCK_CIPHER_SW_HANDLE handle, uint8_t * cipherText, uint32_t * numCipherBytes, uint8_t * plainText, uint32_t numPlainBytes, uint32_t options)
{
    ...
    BLOCK_CIPHER_SW_OFB_CONTEXT * context = (BLOCK_CIPHER_SW_OFB_CONTEXT *)BLOCK_CIPHER_SW_HandleResolve(handle);
    ...
    {
        status = BLOCK_CIPHER_SW_OFB_KeyStreamGenerate (handle, 1, options);
    ...
    while(numPlainBytes--)
    {
        ...
        {
            status = BLOCK_CIPHER_SW_OFB_KeyStreamGenerate   (handle, 1, BLOCK_CIPHER_SW_OPTION_STREAM_CONTINUE);
            if(status != BLOCK_CIPHER_SW_ERROR_NONE)
            {
                if (numCipherBytes != 0)
                {
