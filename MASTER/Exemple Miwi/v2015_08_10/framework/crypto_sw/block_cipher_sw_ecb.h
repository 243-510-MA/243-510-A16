...
#if !defined (_BLOCK_CIPHER_SW_ECB_H)
#define _BLOCK_CIPHER_SW_ECB_H
...
typedef struct
{
    uint8_t __attribute__((aligned)) remainingData[CRYPTO_CONFIG_SW_BLOCK_MAX_SIZE];     // Buffer to store data until more is available if there is not enough to encrypt an entire block.
...
/* Function:
  ...
  Example:
    ...
    error = BLOCK_CIPHER_SW_ECB_Initialize (handle, &context, AES_SW_Encrypt, AES_SW_Decrypt, AES_SW_BLOCK_SIZE, &round_keys, CRYPTO_SW_KEY_SOFTWARE_EXPANDED);
...
/* Function:
  ...
  Example:
    ...
    // Decrypt the data
    BLOCK_CIPHER_SW_ECB_Decrypt (handle, plain_text, &num_bytes_decrypted, (void *) cipher_text, sizeof(cipher_text), BLOCK_CIPHER_SW_OPTION_STREAM_START);
    </code>
*/
