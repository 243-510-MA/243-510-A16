...
#if !defined (_BLOCK_CIPHER_SW_GCM_H)
#define _BLOCK_CIPHER_SW_GCM_H
...
typedef struct
{
    uint8_t __attribute__((aligned)) initializationVector[CRYPTO_CONFIG_SW_BLOCK_MAX_SIZE];  // Buffer containing the initialization vector and initial counter.
    uint8_t __attribute__((aligned)) counter[CRYPTO_CONFIG_SW_BLOCK_MAX_SIZE];               // Buffer containing the current counter value.
    uint8_t __attribute__((aligned)) hashSubKey[CRYPTO_CONFIG_SW_BLOCK_MAX_SIZE];            // Buffer containing the calculated hash subkey
    uint8_t __attribute__((aligned)) authTag[CRYPTO_CONFIG_SW_BLOCK_MAX_SIZE];               // Buffer containing the current authentication tag
    uint8_t __attribute__((aligned)) authBuffer[CRYPTO_CONFIG_SW_BLOCK_MAX_SIZE];            // Buffer containing data that has been encrypted but has not been authenticated
...
/* Function:
  ...
  Example:
    ...
    error = BLOCK_CIPHER_SW_GCM_Initialize (handle, &context, AES_SW_Encrypt, AES_SW_Decrypt, AES_SW_BLOCK_SIZE, (uint8_t *)ivValue, 12, (void *)&keyStream, sizeof(keyStream), &round_keys, CRYPTO_SW_KEY_SOFTWARE_EXPANDED);
...
/***********************************************************************************************************************************************************************************************
  ...
  Description:
    ...
    user should initialize a GCM context using BLOCK_CIPHER_SW_GCM_Initialize,
    then pass all authenticated-but-not-encrypted data into this function
    with the BLOCK_CIPHER_SW_OPTION_AUTHENTICATE_ONLY option, and then pass
    any authenticated-and-encrypted data in using the
    BLOCK_CIPHER_SW_OPTION_STREAM_CONTINUE option. When calling this function
    for the final time, the user must use the
    BLOCK_CIPHER_SW_OPTION_STREAM_COMPLETE option to generate padding required
    to compute the authentication tag successfully. Note that
    BLOCK_CIPHER_SW_OPTION_STREAM_COMPLETE must always be specified at the end
    ...
    BLOCK_CIPHER_SW_OPTION_AUTHENTICATE_ONLY and
  ...
  Conditions:
    ...
    The initializationVector parameter in the BLOCK_CIPHER_SW_GCM_CONTEXT
  ...
  Input:
    ...
    authenticationTag -  Pointer to a structure to contain the
                         ...
                         BLOCK_CIPHER_SW_OPTION_STREAM_COMPLETE option.
    ...
    options -            Block cipher encryption options that the user can
                         ...
                         then BLOCK_CIPHER_SW_OPTION_STREAM_CONTINUE is
                         assumed. Valid options for this function are
                         * BLOCK_CIPHER_SW_OPTION_AUTHENTICATE_ONLY
  ...
  Return:
    Returns a member of the BLOCK_CIPHER_SW_ERRORS enumeration:
      ...
      * BLOCK_CIPHER_SW_ERROR_GCM_COUNTER_EXPIRED - The requesting call
  ...
  Example:
    ...
    if (BLOCK_CIPHER_SW_GCM_Encrypt (handle, cipherText, &numCipherBytes, (uint8_t *)ptShort, 40, tag, 16, BLOCK_CIPHER_SW_OPTION_STREAM_CONTINUE) != BLOCK_CIPHER_SW_ERROR_NONE)
    {
        // An error occured
        while(1);
    }
    ...
    // Since we are using BLOCK_CIPHER_SW_OPTION_STREAM_COMPLETE, we must specify a pointer to and length of the tag array, to store the auth tag.
    if (BLOCK_CIPHER_SW_GCM_Encrypt (handle, cipherText + 40, &numCipherBytes, (uint8_t *)ptShort + 40, 20, tag, 16, BLOCK_CIPHER_SW_OPTION_STREAM_COMPLETE) != BLOCK_CIPHER_SW_ERROR_NONE)
    {
        // An error occured
        while(1);
    }
...
/* Function:
  ...
  Return:
    Returns a member of the BLOCK_CIPHER_SW_ERRORS enumeration:
      ...
      * BLOCK_CIPHER_SW_ERROR_GCM_COUNTER_EXPIRED - The requesting call
        has caused the counter number to run out of unique combinations.
      * BLOCK_CIPHER_SW_ERROR_INVALID_AUTHENTICATION - The calculated
  ...
  Example:
   ...
   // Cipher text to decrypt
    ...
    // Note that at this point, you don't really need to specify the tag pointer or its length.  This parameter only
    // needs to be specified when the BLOCK_CIPHER_SW_OPTION_STREAM_COMPLETE option is used.
    if (BLOCK_CIPHER_SW_GCM_Decrypt (handle, plain_text, &numPlainBytes, (uint8_t *)cipher_text, 40, tag, 16, BLOCK_CIPHER_SW_OPTION_STREAM_CONTINUE) != BLOCK_CIPHER_SW_ERROR_NONE)
    {
        // An error occured
        while(1);
    }
    ...
    // Since we are using BLOCK_CIPHER_SW_OPTION_STREAM_COMPLETE, we must specify the authentication tag and its length.  If it does not match
    // the tag we obtain by decrypting the data, the Decrypt function will return BLOCK_CIPHER_SW_ERROR_INVALID_AUTHENTICATION.
    if (BLOCK_CIPHER_SW_GCM_Decrypt (handle, plain_text + 40, &numPlainBytes, (uint8_t *)cipher_text + 40, 20, tag, 16, BLOCK_CIPHER_SW_OPTION_STREAM_COMPLETE) != BLOCK_CIPHER_SW_ERROR_NONE)
    {
        // An error occured
        while(1);
    }
...
/* Function:
  ...
  Precondition:
    ...
    The initializationVector parameter in the BLOCK_CIPHER_SW_GCM_CONTEXT structure
  ...
  Returns:
    Returns a member of the BLOCK_CIPHER_SW_ERRORS enumeration:
        * BLOCK_CIPHER_SW_ERROR_NONE - no error.
        * BLOCK_CIPHER_SW_ERROR_KEY_STREAM_GEN_OUT_OF_SPACE - There was not enough room 
            remaining in the context->keyStream buffer to fit the key data requested by the 
            numBlocks parameter.
        * BLOCK_CIPHER_SW_ERROR_GCM_COUNTER_EXPIRED - The requesting call has caused the counter
  ...
  Example:
    ...
    error = BLOCK_CIPHER_SW_GCM_Initialize (handle, &context, AES_SW_Encrypt, AES_SW_Decrypt, AES_SW_BLOCK_SIZE, initialization_vector, 12, (void *)&keyStream, sizeof (keyStream), &round_keys);
    ...
    //Encrypt the data
    BLOCK_CIPHER_SW_GCM_Encrypt (handle, cipher_text, &numCipherBytes, (void *) plain_text, sizeof(plain_text), BLOCK_CIPHER_SW_OPTION_STREAM_CONTINUE);
    </code>
*/
